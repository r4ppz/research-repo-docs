...
> The system is ready for testing, but because there is no dedicated server yet, you must contact the developers to try it. There are two ways to do this. First, you can visit the [GitHub Pages SPA Frontend](r4ppz.github.io/research-repository/) and ask the developers to run the backend on their hardware to support your session. Alternatively, you can contact the developers to get the containerized version (.zip file) of the entire system, which includes the frontend, backend, and database in a Docker Compose file (not the wholecodebase but just the compiled/packaged version) for you to run locally.
